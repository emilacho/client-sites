/**
 * Round 37 · trigger anchor remap evidence.
 * Capture qa frozen + 4 default-cam rotation frames per phase.
 * Also captures a hover-test frame: navigate, hover over the chest
 * in screen-space, screenshot to evidence that the new cofre anchor
 * now reads as a click target on the chest.
 */
...
const framePath = join(dir, `round-37-${phase}.png`)
const comboPath = join(dir, "round-37-before-after.png")
...

{
  ...
  for (let i = 0; i < angles.length; i++) {
    ...
    const deg = angles[i]
    const p = join(dir, `round-37-rot-${String(deg).padStart(3, "0")}-${phase}.png`)
...
if (phase === "after") {
  const beforePath = join(dir, "round-37-before.png")
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:8px;">
  <div>
    <div style="padding:6px 10px;font-size:11px;font-family:ui-monospace,monospace;letter-spacing:.18em;color:#a78bfa;text-transform:uppercase;background:#1a1a24;border-radius:6px;margin-bottom:6px;">round 37 · before · cofre on sign · cocos on trunk</div>
    <img src="data:image/png;base64,${b64Before}" style="width:100%;display:block;border-radius:8px;border:1px solid #2a2a35;" />
  </div>
  <div>
    <div style="padding:6px 10px;font-size:11px;font-family:ui-monospace,monospace;letter-spacing:.18em;color:#67e8f9;text-transform:uppercase;background:#1a1a24;border-radius:6px;margin-bottom:6px;">round 37 · after · cofre on Chest_14 · cocos on Coconut_2_5</div>
