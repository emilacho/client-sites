...
const framePath = join(dir, `round-25-${phase}.png`)
const comboPath = join(dir, "round-25-before-after.png")
...

{
  ...
  for (let i = 0; i < angles.length; i++) {
    ...
    const deg = angles[i]
    const p = join(dir, `round-25-rot-${String(deg).padStart(3, "0")}-${phase}.png`)
...
if (phase === "after") {
  const beforePath = join(dir, "round-25-before.png")
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:8px;">
  <div>
    <div style="padding:6px 10px;font-size:11px;font-family:ui-monospace,monospace;letter-spacing:.18em;color:#a78bfa;text-transform:uppercase;background:#1a1a24;border-radius:6px;margin-bottom:6px;">round 25 · before · airgap · isla flotante</div>
    <img src="data:image/png;base64,${b64Before}" style="width:100%;display:block;border-radius:8px;border:1px solid #2a2a35;" />
  </div>
  <div>
    <div style="padding:6px 10px;font-size:11px;font-family:ui-monospace,monospace;letter-spacing:.18em;color:#67e8f9;text-transform:uppercase;background:#1a1a24;border-radius:6px;margin-bottom:6px;">round 25 · after · island emerging from water</div>
