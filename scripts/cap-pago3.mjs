...
await p.evaluate(() => [...document.querySelectorAll("button")].find(e=>/Tarjeta/i.test(e.textContent||""))?.click())
await p.waitForTimeout(3500)
await p.screenshot({ path: join(dir, "05-tarjeta.png") })
const t3 = await p.evaluate(() => document.body.innerText)
console.log("¿pide datos de tarjeta?:", /1234 5678|MM\/AA|COMO APARECE/i.test(t3))
const pagar = await p.evaluate(() => { const b=[...document.querySelectorAll("button")].find(e=>/^Pagar/i.test((e.textContent||"").trim())); return b ? {t:b.textContent.trim(), off:b.disabled} : null })
console.log("boton pagar:", JSON.stringify(pagar))
await b.close()
