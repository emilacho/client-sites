/**
 * Round 12 · sign + surfboard visibility audit + (optional)
 * reposition evidence.
 *
 *   node scripts/capture-round12.mjs --phase=before
 *   node scripts/capture-round12.mjs --phase=after
 *
 * Each phase captures:
 *   - round-12-${phase}.png             · qa=1 frozen front view
 *   - round-12-rot-${deg}-${phase}.png  · live rotation 0/90/180/270
 *
 * After-phase also composites round-12-before-after.png.
 */
...
const framePath = join(dir, `round-12-${phase}.png`)
const comboPath = join(dir, "round-12-before-after.png")
...
// ── 4-angle rotation ──────────────────────────────────────────────────
{
  ...
  for (let i = 0; i < angles.length; i++) {
    ...
    const deg = angles[i]
    const p = join(dir, `round-12-rot-${String(deg).padStart(3, "0")}-${phase}.png`)
...
if (phase === "after") {
  const beforePath = join(dir, "round-12-before.png")
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;padding:8px;">
  <div>
    <div style="padding:6px 10px;font-size:11px;font-family:ui-monospace,monospace;letter-spacing:.18em;color:#a78bfa;text-transform:uppercase;background:#1a1a24;border-radius:6px;margin-bottom:6px;">round 12 · before</div>
    <img src="data:image/png;base64,${b64Before}" style="width:100%;display:block;border-radius:8px;border:1px solid #2a2a35;" />
  </div>
  <div>
    <div style="padding:6px 10px;font-size:11px;font-family:ui-monospace,monospace;letter-spacing:.18em;color:#67e8f9;text-transform:uppercase;background:#1a1a24;border-radius:6px;margin-bottom:6px;">round 12 · after</div>
