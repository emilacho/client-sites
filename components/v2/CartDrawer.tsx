...
/* R97.5 v2 · MockPaymentForm · réplica del pattern checkout Domino's
 *  Pizza adaptado a Náufrago (purple #3D2466 + cyan #4DD4D8) ·
 *
 *  - Header section "Pago"
 *  - Order summary recap (subtotal · envío · total)
 *  - Payment method selector tabs (Crédito/Débito · Efectivo)
 *  - Credit/Debit form ·
 *      Card number con auto-detect brand (Visa/MC/Amex/Discover · logo inline)
 *      Vencimiento MM/AA
 *      CVV con tooltip help
 *      Titular tarjeta
 *      "Guardar tarjeta" checkbox (mock)
 *  - Trust line · 🔒 Pago seguro encriptado · brand logos aceptados
 *  - Big purple CTA "Pagar $X.XX" + back link
 *  - Cash on Delivery alt option · skip form · directo a confirmar
 *
 *  Cuando Kushki real esté integrado · este componente se reemplaza
 *  por su iframe SDK · interface onPay queda igual.
 */
...
function CardBrandLogo({ brand }: { brand: CardBrand }) {
  ...
  if (brand === "unknown") {
    return (
      <span
        className="flex h-6 w-10 items-center justify-center rounded text-[10px]"
  ...
  return (
    <span
      className="flex h-6 w-10 items-center justify-center rounded text-[10px] font-extrabold tracking-wider"
...
function MockPaymentForm({
  ...
  const canPay = method === "cash" || cardValid
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Order summary recap (compact) */}
      <div
        className="rounded-2xl px-3 py-2.5 text-[12px]"
        style={{
          background: "rgba(245,233,210,0.08)",
          border: "1px solid rgba(245,233,210,0.15)",
        }}
      >
        <div className="mb-1.5 flex items-baseline justify-between">
          ...
          <span className="text-[10px] text-slate-500">
            {cart.itemCount} {cart.itemCount === 1 ? "plato" : "platos"}
          </span>
        </div>
        <div className="space-y-1">
          ...
          {discount > 0 ? (
            <div className="flex justify-between" style={{ color: "#4DD4D8" }}>
              <span>Descuento</span>
              <span className="tabular-nums">−${discount.toFixed(2)}</span>
            </div>
          ) : null}
          <div className="flex justify-between text-slate-300">
            <span>Envío · {etaMinutes} min</span>
            <span className="tabular-nums">${priceUsd.toFixed(2)}</span>
          </div>
          {cart.tipUsd > 0 ? (
            <div className="flex justify-between text-slate-300">
              <span>Propina motorizado</span>
          ...
          <div
            className="mt-1.5 flex items-baseline justify-between border-t pt-1.5 font-semibold"
            style={{ borderColor: "rgba(245,233,210,0.15)", color: "#F5E9D2" }}
          >
      ...
      {/* Method selector · tabs estilo Domino's */}
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={[
              "flex items-center justify-center gap-2 rounded-xl border-2 py-2.5 text-xs font-semibold transition-all",
              method === "card"
      ...
      {method === "card" ? (
        ...
        >
          ...
          {/* Card number con brand auto-detect */}
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <label className="block">
              <span className="mb-1 flex items-center gap-1 text-[11px] font-medium text-slate-300">
                CVV
                <button
                  type="button"
                  onClick={() => setShowCvvHelp((v) => !v)}
                  aria-label="¿Qué es el CVV?"
          ...
          {showCvvHelp ? (
            <p
              ...
              style={{ background: "rgba(77,212,216,0.10)", color: "#4DD4D8" }}
            >
              CVV · {brand === "amex" ? "4 dígitos al frente" : "3 dígitos al reverso"} de la tarjeta.
            </p>
          ) : null}
        ...
        <div
          className="space-y-1.5 rounded-2xl border-2 px-3 py-4 text-center"
          style={{
            background: "linear-gradient(180deg, rgba(61,36,102,0.20) 0%, rgba(31,17,56,0.30) 100%)",
            borderColor: "rgba(77,212,216,0.30)",
          }}
        >
          ...
          <p className="text-[11px] text-slate-400">
            El motorizado pasa con el cambio · prepará $
            {totalUsd.toFixed(2)} exactos cuando llegue.
          </p>
        </div>
      )}
      ...
      <div className="flex items-center justify-center gap-1.5 text-[10px] text-slate-500">
        <span>🔒</span>
        <span>Pago seguro encriptado · Kushki Ecuador</span>
      </div>
      ...
      <button
        ...
        style={{
          background: canPay
            ? "linear-gradient(180deg, #4DD4D8 0%, #2BA8AC 100%)"
            : "rgba(60,60,60,0.5)",
          boxShadow: canPay
            ? "0 14px 32px -12px rgba(77,212,216,0.55)"
            : "none",
          color: canPay ? "#1F1138" : "rgba(255,255,255,0.4)",
        }}
