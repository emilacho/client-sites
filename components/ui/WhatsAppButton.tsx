...
export function WhatsAppButton({
  ...
  const sizeClass = size === "lg" ? "h-12 px-7 text-base" : "h-10 px-5 text-sm"
  return (
    <a
      href={whatsappLink(message)}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-flex items-center justify-center gap-2 rounded-full bg-[#25d366] font-semibold text-white shadow-sm transition-all hover:bg-[#1ebe57] hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#25d366]/40 ${sizeClass} ${className}`.trim()}
    >
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
